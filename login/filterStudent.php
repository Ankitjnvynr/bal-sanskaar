...
$sql = "SELECT COUNT(*) AS total FROM students WHERE `district` = '$userDistrict' AND `tehsil` = '$userTehsil' AND `center`= '$center' ";
if ($search) {
...
$sql = "SELECT * FROM students WHERE `district` = '$userDistrict' AND `tehsil` = '$userTehsil' AND `center`= '$center' ";
if ($search) {
    $sql .= " AND (`name` LIKE '%$search%' OR `father_name` LIKE '%$search%' OR `mother_name` LIKE '%$search%')";
}
        ...
        <div class="overflow-x-scroll">
            <form method="GET" action="">
                <div class="input-group">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" class="form-control mb-3" placeholder="Search...">
                    <input type="hidden" name="page" value="1">
                    <input type="hidden" name="center" value="<?php echo $_GET['center'] ?>">
                    <input type="hidden" name="data" value="filterStudent">
